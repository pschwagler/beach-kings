...
def test_player_stats_init():
    ...
    stats = calculation_service.PlayerStats("Alice")
...
def test_player_stats_record_methods():
    ...
    # Record partnerships
    stats.record_game_with("Bob")
    stats.record_win_with("Bob")
    assert stats.games_with["Bob"] == 1
    assert stats.wins_with["Bob"] == 1
    
    # Record opponents
    stats.record_game_against("Charlie")
    stats.record_win_against("Charlie")
    assert stats.games_against["Charlie"] == 1
    assert stats.wins_against["Charlie"] == 1
    
    # Record point differentials
    stats.record_point_diff_with("Bob", 5)
    stats.record_point_diff_against("Charlie", -3)
    assert stats.point_diff_with["Bob"] == 5
    assert stats.point_diff_against["Charlie"] == -3
...
def test_stats_tracker_get_player():
    ...
    # Get new player
    player = tracker.get_player("Alice")
    assert player.name == "Alice"
    assert "Alice" in tracker.players
    
    # Get existing player
    player2 = tracker.get_player("Alice")
...
def create_mock_match(team1_players, team2_players, team1_score, team2_score, match_id=1, date=None):
    ...
    class MockMatch:
        def __init__(self, players, team1_score, team2_score, match_id, date):
...
def test_stats_tracker_process_match_win():
    ...
    # Check all players were created
    assert "Alice" in tracker.players
    assert "Bob" in tracker.players
    assert "Charlie" in tracker.players
    assert "Dave" in tracker.players
    
    # Check game counts
    for player_name in ["Alice", "Bob", "Charlie", "Dave"]:
        assert tracker.players[player_name].game_count == 1
    
    # Check win counts (team 1 won)
    assert tracker.players["Alice"].win_count == 1
    assert tracker.players["Bob"].win_count == 1
    assert tracker.players["Charlie"].win_count == 0
    assert tracker.players["Dave"].win_count == 0
    
    # Check partnerships
    assert tracker.players["Alice"].games_with["Bob"] == 1
    assert tracker.players["Bob"].games_with["Alice"] == 1
    assert tracker.players["Charlie"].games_with["Dave"] == 1
    assert tracker.players["Dave"].games_with["Charlie"] == 1
    
    # Check opponent records
    alice = tracker.players["Alice"]
    assert "Charlie" in alice.games_against
    assert "Dave" in alice.games_against
    assert alice.games_against["Charlie"] == 1
    assert alice.games_against["Dave"] == 1
...
def test_stats_tracker_process_match_tie():
    ...
    for player_name in ["Alice", "Bob", "Charlie", "Dave"]:
        assert tracker.players[player_name].game_count == 1
        # No wins in a tie
        assert tracker.players[player_name].win_count == 0
...
def test_stats_tracker_multiple_matches():
    ...
    # Match 1: Alice & Bob beat Charlie & Dave
    match1 = create_mock_match(["Alice", "Bob"], ["Charlie", "Dave"], 21, 19, match_id=1)
    tracker.process_match(match1)
    
    # Match 2: Alice & Charlie beat Bob & Dave
    match2 = create_mock_match(["Alice", "Charlie"], ["Bob", "Dave"], 21, 17, match_id=2)
    ...
    # Check partnerships
    assert alice.games_with["Bob"] == 1
    assert alice.games_with["Charlie"] == 1
    
    # Check Bob's stats
    bob = tracker.players["Bob"]
...
def test_process_matches_integration():
    ...
    # Create mock matches
    matches = [
        create_mock_match(["Alice", "Bob"], ["Charlie", "Dave"], 21, 19, match_id=1),
        create_mock_match(["Alice", "Charlie"], ["Bob", "Dave"], 21, 17, match_id=2),
    ]
    ...
    # Process matches
    elo_deltas_map, partnerships, opponents, elo_history = calculation_service.process_matches(
        matches,
        player_id_map
    )
    
    # Check results
    assert len(elo_deltas_map) == 2
    ...
    for eh in elo_history:
        assert eh.player_id in player_id_map.values()
