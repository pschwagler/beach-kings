...
class TestAuthEndpoints:
    ...
    def test_signup_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            user_service, "check_phone_exists", fake_check_phone_exists, raising=True
        )
        monkeypatch.setattr(
            user_service, "create_verification_code", fake_create_verification_code, raising=True
        )
    ...
    def test_verify_phone_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            user_service, "verify_and_mark_code_used", fake_verify_and_mark_code_used, raising=True
        )
        monkeypatch.setattr(
            user_service, "check_phone_exists", fake_check_phone_exists, raising=True
        )
        ...
        monkeypatch.setattr(
            user_service, "reset_failed_attempts", fake_reset_failed_attempts, raising=True
        )
        monkeypatch.setattr(
            user_service, "create_refresh_token", fake_create_refresh_token, raising=True
        )
    ...
    def test_verify_phone_invalid_code(self, monkeypatch):
        ...
        monkeypatch.setattr(
            user_service, "increment_failed_attempts", fake_increment_failed_attempts, raising=True
        )

        payload = {"phone_number": "+15551234567", "code": "000000"}
        response = client.post("/api/auth/verify-phone", json=payload)
        assert response.status_code == 401
    ...
    def test_refresh_token_success(self, monkeypatch):
        ...
        async def fake_get_refresh_token(session, token):
            ...
            return {"user_id": 1, "token": token, "expires_at": expires_at}
...
class TestPlayerEndpoints:
    ...
    def test_create_player(self, monkeypatch):
        ...
        monkeypatch.setattr(
            data_service, "get_or_create_player", fake_get_or_create_player, raising=True
        )
    ...
    def test_get_user_player(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            fake_get_player_by_user_id_with_stats,
            raising=True,
        )
    ...
    def test_update_user_player(self, monkeypatch):
        ...
        payload = {"full_name": "Updated Name", "level": "AA"}
...
class TestPhotoJobStreamEndpoint:
    ...
    def test_stream_requires_auth(self):
        """Stream endpoint returns 401 without auth (HTTPBearer auto_error)."""
        client = TestClient(app)
    ...
    def test_stream_200_partial_then_done(self, monkeypatch):
        ...
        monkeypatch.setattr(
            photo_match_service, "stream_photo_job_events", fake_stream_events, raising=True
        )
        monkeypatch.setattr(
            auth_dependencies, "_has_league_role", fake_has_league_role, raising=True
        )
