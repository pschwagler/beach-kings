...
async def test_calculate_global_stats_single_match(db_session, test_players, test_session):
    ...
    partnership_result = await db_session.execute(
        select(PartnershipStats)
    )
    partnerships = partnership_result.scalars().all()
    
    # Should have Alice-Bob and Charlie-Dave partnerships
    alice_bob = next((p for p in partnerships if p.player_id == alice.id and p.partner_id == bob.id), None)
    charlie_dave = next((p for p in partnerships if p.player_id == charlie.id and p.partner_id == dave.id), None)
    ...
    opponent_result = await db_session.execute(
        select(OpponentStats)
    )
    opponents = opponent_result.scalars().all()
    
    # Alice should have stats against Charlie and Dave
    alice_vs_charlie = next((o for o in opponents if o.player_id == alice.id and o.opponent_id == charlie.id), None)
    alice_vs_dave = next((o for o in opponents if o.player_id == alice.id and o.opponent_id == dave.id), None)
...
async def test_calculate_global_stats_multiple_matches(db_session, test_players, test_session):
    ...
    partnership_result = await db_session.execute(
        select(PartnershipStats)
        .where(PartnershipStats.player_id == alice.id)
    )
    ...
    opponent_result = await db_session.execute(
        select(OpponentStats)
        .where(OpponentStats.player_id == alice.id)
    )
...
async def test_calculate_season_stats(db_session, test_players, test_league_and_season, test_session):
    ...
    opponent_result = await db_session.execute(
        select(OpponentStatsSeason)
        .where(OpponentStatsSeason.season_id == season.id)
    )
...
@pytest.mark.asyncio
async def test_global_vs_season_stats_separation(db_session, test_players, test_league_and_season, test_session):
    ...
    global_partnerships_result = await db_session.execute(
        select(PartnershipStats)
    )
    ...
    global_alice_bob = next(
        (p for p in global_partnerships_after_list 
         if p.player_id == alice.id and p.partner_id == bob.id),
        None
    )
    season_alice_bob = next(
        (p for p in season_partnerships_list
         if p.player_id == alice.id and p.partner_id == bob.id),
        None
    )
...
async def test_elo_calculation_accuracy(db_session, test_players, test_session):
    ...
    elo_change_team1 = calculation_service.elo_change(
        K, team1_avg_elo, expected_score_team1, 1.0
    )
...
async def test_stats_recalculation_removes_stale_data(db_session, test_players, test_session):
    ...
    partnership1 = await db_session.execute(
        select(PartnershipStats)
        .where(
            PartnershipStats.player_id == alice.id,
            PartnershipStats.partner_id == bob.id
        )
    )
    ...
    alice_charlie = await db_session.execute(
        ...
        .where(
            PartnershipStats.player_id == alice.id,
            PartnershipStats.partner_id == charlie.id
        )
    )
...
@pytest.mark.asyncio
async def test_multiple_seasons_separate_stats(db_session, test_players, test_league_and_season, test_session):
    ...
    season1_stats = await db_session.execute(
        select(PartnershipStatsSeason)
        .where(PartnershipStatsSeason.season_id == season1.id)
    )
    ...
    season2_stats = await db_session.execute(
        select(PartnershipStatsSeason)
        .where(PartnershipStatsSeason.season_id == season2.id)
    )
    ...
    season2_alice_charlie = next(
        (p for p in season2_partnerships if p.player_id == alice.id and p.partner_id == charlie.id),
        None
    )
...
@pytest.mark.asyncio
async def test_season_calculation_only_includes_season_matches(db_session, test_players, test_league_and_season):
    ...
    match1 = await create_match(
        db_session, session1, alice, bob, charlie, dave, 21, 19, is_ranked=True
    )
    match2 = await create_match(
    ...
    elo_history = await db_session.execute(
        select(EloHistory).join(Match, EloHistory.match_id == Match.id)
    ...
    alice_charlie = next(
        (p for p in season1_partnerships if p.player_id == alice.id and p.partner_id == charlie.id),
        None
    )
...
async def test_partnership_stats_bidirectional(db_session, test_players, test_session):
    ...
    alice_bob = next(
        (p for p in partnerships if p.player_id == alice.id and p.partner_id == bob.id),
        None
    )
    assert alice_bob is not None
    ...
    bob_alice = next(
        (p for p in partnerships if p.player_id == bob.id and p.partner_id == alice.id),
        None
    )
...
async def test_opponent_stats_bidirectional(db_session, test_players, test_session):
    ...
    alice_vs_charlie = next(
        (o for o in opponents if o.player_id == alice.id and o.opponent_id == charlie.id),
        None
    )
    assert alice_vs_charlie is not None
    assert alice_vs_charlie.games == 1
    ...
    charlie_vs_alice = next(
        (o for o in opponents if o.player_id == charlie.id and o.opponent_id == alice.id),
        None
    )
