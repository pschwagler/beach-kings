...
async def test_create_user_duplicate_phone(test_session):
    ...
    await user_service.create_user(
        session=test_session, phone_number="+15551234567", password_hash="hash1"
    )

    # Try to create duplicate
    with pytest.raises(ValueError, match="already registered"):
        await user_service.create_user(
            session=test_session, phone_number="+15551234567", password_hash="hash2"
        )
...
async def test_get_user_by_phone(test_session):
    ...
    # Create user
    user_id = await user_service.create_user(
        session=test_session, phone_number="+15551234567", password_hash="hash"
    )
...
async def test_verify_and_mark_code_used(test_session):
    ...
    signup_data = await user_service.verify_and_mark_code_used(
        session=test_session, phone_number="+15551234567", code="123456"
    )
...
async def test_verify_code_wrong_code(test_session):
    ...
    # Wrong code
    signup_data = await user_service.verify_and_mark_code_used(
        session=test_session, phone_number="+15551234567", code="999999"
    )
...
async def test_update_user_password(test_session):
    ...
    # Create user
    user_id = await user_service.create_user(
        session=test_session, phone_number="+15551234567", password_hash="old_hash"
    )

    # Update password
    success = await user_service.update_user_password(
        session=test_session, user_id=user_id, password_hash="new_hash"
    )
...
async def test_create_refresh_token(test_session):
    ...
    success = await user_service.create_refresh_token(
        session=test_session, user_id=user_id, token="refresh_token_123", expires_at=expires_at
    )
