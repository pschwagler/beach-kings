...
def test_locations_crud_system_admin(monkeypatch):
    ...
    r = client.post(
        "/api/locations",
        json={"id": loc_id, "name": "LA", "city": "Los Angeles", "state": "CA"},
        headers=headers,
    )
    ...
    assert r.status_code == 200
    assert any(loc["id"] == loc_id for loc in r.json())
    # update
...
def test_courts_crud_system_admin(monkeypatch):
    ...
    loc = client.post(
        "/api/locations",
        json={"id": loc_id, "name": "OC", "city": "Newport", "state": "CA"},
        headers=headers,
    ).json()
    assert loc["id"] == loc_id
    # create court
    r = client.post(
        "/api/courts", json={"name": "Court A", "location_id": loc_id}, headers=headers
    )
    assert r.status_code == 200
    court = r.json()
