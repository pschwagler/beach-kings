...
def upgrade() -> None:
    ...
    if "player_global_stats" in tables:
        try:
            ...
            if "idx_player_global_stats_player" not in indexes:
                op.create_index(
                    "idx_player_global_stats_player",
                    "player_global_stats",
                    ["player_id"],
                    unique=False,
                )
    ...
    if "signup_players" in tables:
        try:
            ...
            has_constraint = any(
                set(c["column_names"]) == {"signup_id", "player_id"} for c in unique_constraints
            )
            if not has_constraint:
                op.create_unique_constraint(
                    "uq_signup_players_signup_player", "signup_players", ["signup_id", "player_id"]
                )
        except Exception:
