...
def _add_visibility(table: str) -> None:
    ...
    _create_index_if_missing(
        f"idx_{table}_moderation_visibility", table, ["moderation_visibility"]
    )
...
def upgrade() -> None:
    if not _has_table("user_blocks"):
        op.create_table(
            ...
            sa.UniqueConstraint(
                "blocker_player_id", "blocked_player_id", name="uq_user_blocks_pair"
            ),
            sa.CheckConstraint(
                "blocker_player_id <> blocked_player_id", name="ck_user_blocks_not_self"
            ),
        )
    _create_index_if_missing(
        "idx_user_blocks_blocked", "user_blocks", ["blocked_player_id", "blocker_player_id"]
    )
    ...
    if not _has_table("interaction_restrictions"):
        op.create_table(
            ...
            sa.CheckConstraint(
                "expires_at > starts_at", name="ck_interaction_restrictions_window"
            ),
        )
    _create_index_if_missing(
        "idx_interaction_restrictions_active",
        "interaction_restrictions",
    ...
    if not _has_table("moderation_cases"):
        op.create_table(
            ...
            sa.Column(
                "subject_player_id", sa.Integer(), sa.ForeignKey("players.id", ondelete="SET NULL")
            ),
    ...
    _create_index_if_missing(
        "idx_moderation_cases_queue", "moderation_cases", ["state", "severity", "due_at"]
    )
    _create_index_if_missing(
        "idx_moderation_cases_target", "moderation_cases", ["target_type", "target_id"]
    )
    ...
    _create_index_if_missing(
        "idx_moderation_events_case", "moderation_events", ["case_id", "created_at"]
    )
    op.execute(
        """
    ...
    if not _has_table("moderation_jobs"):
        op.create_table(
            ...
            sa.Column(
                "case_id", sa.Integer(), sa.ForeignKey("moderation_cases.id", ondelete="CASCADE")
            ),
            sa.Column("target_type", sa.String(40), nullable=False),
            sa.Column("target_id", sa.Integer(), nullable=False),
    ...
    _create_index_if_missing(
        "idx_moderation_jobs_claim", "moderation_jobs", ["status", "available_at"]
    )
    ...
    _create_index_if_missing(
        "idx_moderation_evidence_purge", "moderation_evidence", ["purge_after", "purged_at"]
    )
