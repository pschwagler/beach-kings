...
def upgrade() -> None:
    ...
    if not _table_exists(conn, "player_league_stats"):
        op.create_table(
            ...
            sa.UniqueConstraint(
                "player_id", "league_id", name="uq_player_league_stats_player_league"
            ),
        )
    ...
    if not _table_exists(conn, "partnership_stats_league"):
        ...
        if not _index_exists(conn, "idx_partnership_stats_league_player"):
            op.create_index(
                "idx_partnership_stats_league_player", "partnership_stats_league", ["player_id"]
            )
        if not _index_exists(conn, "idx_partnership_stats_league_partner"):
            op.create_index(
                "idx_partnership_stats_league_partner", "partnership_stats_league", ["partner_id"]
            )
        if not _index_exists(conn, "idx_partnership_stats_league_league"):
            op.create_index(
                "idx_partnership_stats_league_league", "partnership_stats_league", ["league_id"]
            )
    ...
    if not _table_exists(conn, "opponent_stats_league"):
        ...
        if not _index_exists(conn, "idx_opponent_stats_league_player"):
            op.create_index(
                "idx_opponent_stats_league_player", "opponent_stats_league", ["player_id"]
            )
        if not _index_exists(conn, "idx_opponent_stats_league_opponent"):
            op.create_index(
                "idx_opponent_stats_league_opponent", "opponent_stats_league", ["opponent_id"]
            )
        if not _index_exists(conn, "idx_opponent_stats_league_league"):
            op.create_index(
                "idx_opponent_stats_league_league", "opponent_stats_league", ["league_id"]
            )
    ...
    if not _column_exists(conn, "stats_calculation_jobs", "league_id"):
        op.add_column(
            "stats_calculation_jobs", sa.Column("league_id", sa.Integer(), nullable=True)
        )
        if not _constraint_exists(conn, "fk_stats_calculation_jobs_league"):
            op.create_foreign_key(
                "fk_stats_calculation_jobs_league",
        ...
        if not _index_exists(conn, "idx_stats_calculation_jobs_type_league"):
            op.create_index(
                "idx_stats_calculation_jobs_type_league",
                "stats_calculation_jobs",
                ["calc_type", "league_id"],
            )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_stats_calculation_jobs_league", "stats_calculation_jobs", type_="foreignkey"
    )
