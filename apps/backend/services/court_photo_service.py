...
def _process_image_bytes(content: bytes, content_type: str) -> bytes:
    """
    Synchronous PIL processing: validate, resize, and convert image bytes to JPEG.
...
async def process_court_photo(file: UploadFile) -> bytes:
    ...
    Reads file content asynchronously, then offloads CPU-bound PIL
    processing to a thread to avoid blocking the event loop.
    ...
    return await asyncio.to_thread(_process_image_bytes, content, ct)
