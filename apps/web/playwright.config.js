...
export default defineConfig({
  ...
  webServer: {
    command: `rm -rf .next/cache/fetch-cache 2>/dev/null; rm -f .next/dev/lock 2>/dev/null || true && PORT=3002 BACKEND_PROXY_TARGET=${process.env.TEST_API_URL || 'http://localhost:8001'} BACKEND_INTERNAL_URL=${process.env.TEST_API_URL || 'http://localhost:8001'} npm run dev`,
