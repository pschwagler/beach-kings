...
export function generateTestPhoneNumber() {
  ...
  // Keep all generated accounts in an unmistakable cleanup namespace:
  // +1 202 5xx xxxx. The six random digits provide one million candidates,
  // which avoids collisions when signup specs run in parallel.
  const e2ePrefix = '2025';
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    const phoneNumber = `+1${e2ePrefix}${suffix}`;
 ...
 * @returns {object} { items, total_count, has_more }
 */
...
export async function createTestSession(token, leagueId, sessionData = {}) {
  ...
  try {
    // Session creation moved from /api/leagues/:id/sessions (GET-only now,
    // returns 405) to POST /api/sessions with league_id in the body.
    const response = await api.post('/api/sessions', { ...defaultSessionData, league_id: leagueId });
