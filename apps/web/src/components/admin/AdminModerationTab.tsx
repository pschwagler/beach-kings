...
function CasePanel({ value, onChanged, onError }: { value: CaseDetail; onChanged: () => void; onError: (value: string | null) => void }) {
  const [action, setAction] = useState<string>(''); const [reason, setReason] = useState(''); const [playerMessage, setPlayerMessage] = useState(''); const [lockHours, setLockHours] = useState(24);
  ...
  const submit = async () => {
    if (!action || !reason.trim() || (action === 'warn' && !playerMessage.trim()) || (needsConfirmation && !confirmed) || (needsAppeal && appealId == null)) return;
    const submittedAction = action;
    const submittedReason = reason.trim();
    const submittedPlayerMessage = playerMessage.trim();
    const submittedAppealId = appealId;
    const submittedLockHours = lockHours;
    setMutating(true); onError(null); setSuccess(null);
    try {
      await applyModerationAction(value.id, { action: submittedAction, reason: submittedReason, ...(submittedAction === 'warn' ? { player_message: submittedPlayerMessage } : {}), ...((submittedAction === 'interaction_lock' || submittedAction === 'account_suspend') ? { lock_hours: submittedLockHours } : {}), ...(submittedAction === 'legal_hold' ? { legal_hold: !value.legal_hold } : {}), ...(needsAppeal && submittedAppealId != null ? { appeal_id: submittedAppealId } : {}) });
      setSuccess(`${LABELS[submittedAction]} recorded.`); setReason(''); setPlayerMessage(''); setConfirmed(false); onChanged();
