...
### S1. Back button returns to Home tab instead of originating tab  ✅ confirmed in code
...
- **Fix:** Structural — restructure the root navigator (nest `(tabs)` inside a shared `<Stack>`) so back-history is preserved. Patching individual `backFallback` values is whack-a-mole. Shared pieces: `ui/BackButton.tsx`, `hooks/useBack.ts`.

### S2. Avatar background color differs for the same player across screens  ✅ confirmed in code
...
- **Fix:** Structural — one centralized decision: either a `getAvatarVariant(player)` helper or standardize on a single variant, applied across all call sites. Cheap code, but it IS the shared mechanism.
