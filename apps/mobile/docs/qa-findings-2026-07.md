...
### S3. Leagues list "No games yet this season" despite active standings  ✅ DONE (root-caused: copy bug, not season mismatch — 2026-07-05)
...
- **Root cause (confirmed against live DB):** NOT the hypothesized cross-endpoint season mismatch — `get_user_leagues` resolves the current season correctly (QBK → Season 4) and its whole payload is deliberately **viewer-scoped**: `games_played` counts the *user's* games; `standings` is just the user's own row. Season 4 has 2 matches / 7 stats rows — none involving the viewer — so `games_played: 0` is correct data. The bug was the card copy "No games yet this season", which reads as a league-level claim.
- **Fix:** ✅ Shipped — copy changed to "You haven't played this season yet" (`LeagueCard.tsx`, scoping comment added; tests updated). Verified on-device.
- **Optional follow-up (backlog):** the card could additionally show a league-level activity hint (e.g. top-3 standings preview) for members who haven't played — needs a league-scoped field added to `get_user_leagues`.
