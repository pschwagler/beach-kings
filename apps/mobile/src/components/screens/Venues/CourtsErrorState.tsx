/**
 * Error state for the Courts list when data fetching fails.
 */
...
interface CourtsErrorStateProps {
  readonly onRetry: () => void;
}

export default function CourtsErrorState({
  onRetry,
}: CourtsErrorStateProps): React.ReactNode {
  ...
  return (
    ...
    >
      ...
      <Text className="text-[14px] text-text-muted dark:text-content-tertiary text-center leading-[1.5] mb-8">
        Something went wrong while fetching courts. Check your connection and try
        again.
      </Text>

      <Pressable
        testID="courts-retry-btn"
        onPress={handleRetry}
