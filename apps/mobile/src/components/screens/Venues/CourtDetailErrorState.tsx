/**
 * Error state for the Court Detail screen when data fetching fails.
 */
...
interface CourtDetailErrorStateProps {
  readonly onRetry: () => void;
}

export default function CourtDetailErrorState({
  onRetry,
}: CourtDetailErrorStateProps): React.ReactNode {
  ...
  return (
    <View
      testID="court-detail-error-state"
    ...
    >
      ...
      <Text className="text-[14px] text-text-muted dark:text-content-tertiary text-center leading-[1.5] mb-8">
        Something went wrong while fetching this court. Check your connection and
        try again.
      </Text>

      <Pressable
        testID="court-detail-retry-btn"
        onPress={handleRetry}
