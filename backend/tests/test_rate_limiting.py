...
def mock_user_service(monkeypatch):
    ...
    monkeypatch.setattr(auth_service, "generate_verification_code", fake_generate_verification_code)
    ...
    with patch.object(user_service, "verify_and_mark_code_used", fake_verify_and_mark_code_used):
        ...
        for i in range(10):
            response = client.post(
                ...
                json={"phone_number": phone1, "code": "0000"}
            )
        ...
        for i in range(10):
            response = client.post(
                "/api/auth/reset-password-verify",
                json={"phone_number": phone2, "code": "0000"}
            )
            assert response.status_code == 401, f"Request {i+1} should fail with invalid code"
...
def test_reset_password_normalized_phone_numbers(client, mock_user_service):
    ...
    for format_variant in phone_formats * 3:  # Try each format 3 times = 12 requests
        response = client.post(
            "/api/auth/reset-password",
            json={"phone_number": format_variant}
        )
        request_count += 1
        if request_count <= 10:
            assert response.status_code == 200, f"Request {request_count} with format {format_variant} should succeed"
        else:
            # After 10 requests, should be rate limited regardless of format
            assert response.status_code == 429, f"Request {request_count} should be rate limited (same phone number)"
            break
